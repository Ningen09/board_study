...
public class BoardController {
	...
	public String getBoardListPage(Model model, @RequestParam(required = false, defaultValue =
			"0") Integer page, @RequestParam(required = false, defaultValue = "5") Integer size) throws
	Exception{
	...
	public String boardWriteAction(Model model, BoardRequestDto boardRequestDto) throws Exception{
		try {
			...
			if(result < 0) {
				throw new Exception("#Exception board Write Action");
			}
		}
		catch(Exception e) {
			throw new Exception(e.getMessage());
